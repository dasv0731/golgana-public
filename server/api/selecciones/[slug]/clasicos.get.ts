...
export default defineEventHandler(async (event) => {
  ...
  if (config.useBackend && config.cmsApiUrl) {
    ...
    return client.get(`/selecciones/${slug}/clasicos`);
  }
  try {
    return await loadContent(`selecciones/${slug}/clasicos`);
  } catch {
    throw createError({ statusCode: 404, statusMessage: 'Not Found', message: `Clasicos not found for: ${slug}` });
  }
});
