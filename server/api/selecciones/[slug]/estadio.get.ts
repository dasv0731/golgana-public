...
export default defineEventHandler(async (event) => {
  ...
  if (config.useBackend && config.cmsApiUrl) {
    ...
    return client.get(`/selecciones/${slug}/estadio`);
  }
  try {
    return await loadContent(`selecciones/${slug}/estadio`);
  } catch {
    throw createError({ statusCode: 404, statusMessage: 'Not Found', message: `Estadio not found for: ${slug}` });
  }
});
