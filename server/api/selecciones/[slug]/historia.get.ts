...
export default defineEventHandler(async (event) => {
  ...
  if (config.useBackend && config.cmsApiUrl) {
    ...
    return client.get(`/selecciones/${slug}/historia`);
  }
  try {
    return await loadContent(`selecciones/${slug}/historia`);
  } catch {
    throw createError({ statusCode: 404, statusMessage: 'Not Found', message: `Historia not found for: ${slug}` });
  }
});
