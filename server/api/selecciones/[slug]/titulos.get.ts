...
export default defineEventHandler(async (event) => {
  ...
  if (config.useBackend && config.cmsApiUrl) {
    ...
    return client.get(`/selecciones/${slug}/titulos`);
  }
  try {
    return await loadContent(`selecciones/${slug}/titulos`);
  } catch {
    throw createError({ statusCode: 404, statusMessage: 'Not Found', message: `Titulos not found for: ${slug}` });
  }
});
